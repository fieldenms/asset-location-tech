        ...
        if (data instanceof final MachineActorStarted info) {
            if (!notStartedMachinesKeys.isEmpty()) {
                ...
                if (notStartedMachinesKeys.contains(info.getKey())) {
                    ...
                    LOGGER.info("\t\t" + startedMachinesCount + " / " + machinesKeys.size() + " [" + info.getKey() + "] => " + notStartedMachinesKeys.size() + " machines left ["
                            + some(notStartedMachinesKeys) + "]");
                    if (notStartedMachinesKeys.isEmpty()) {
                ...
                if (!machinesKeys.contains(info.getKey())) {
                    ...
                    LOGGER.info("\t\t" + startedMachinesCount + " / " + machinesKeys.size() + " [" + info.getKey() + " -- " + info.getDesc()
                            + "] => Additional machine actor has been created after registration of new machine.");
                } else {
                    LOGGER.warn("\t\t" + startedMachinesCount + " / " + machinesKeys.size() + " [" + info.getKey() + " -- " + info.getDesc()
                            + "] => Existing machine actor has been died and restarted.");
                }
            }
        } else {
