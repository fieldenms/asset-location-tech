...
public class AvlFrameDecoder extends FrameDecoder {
    ...
    protected static final Logger LOGGER = getLogger(AvlFrameDecoder.class);
    ...
    protected Object decode(final ChannelHandlerContext ctx, final Channel channel, final ChannelBuffer buffer) throws Exception {
        ...
        case DATA:
            LOGGER.debug("available data count [" + availableData + "]");
            if (availableData < 4 + 4) {
                LOGGER.debug("Not enoght header data");
                return null;
            }
            ...
            if (availableData < 4 + 4 + 4 + len) {
                LOGGER.debug("Not enoght packet data");
            ...
            case 0x8:
                // FM4100 & FM2100 codec
                LOGGER.debug("FM4100 & FM2100 codec data");
                ...
                // skip CRC
                buffer.skipBytes(4);
                LOGGER.debug("AVL data count [" + avlArray.length + "]");
                return avlArray;
            default:
                // report unsupported codec and skip data
                LOGGER.info("skip codec = [" + codec + "]");
                final int skipped = -1 + len + 4;
                buffer.skipBytes(skipped);
                LOGGER.debug("skipped bytes count [" + skipped + "]");
                return false;
            }
        case DISCONNECTED:
            // report state
